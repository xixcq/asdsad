...
exports.run = (client, message) => {
  if (!message.member.hasPermission("ADMINISTRATOR")) return message.reply(`:fire: Yeterli yetki, bulunmamakta!`);
    ...
    //if(secenekler === "aç" || "kapat") return message.channel.send(errembed);
      if(secenekler.length < 1) return message.reply("Aktif hale getirmek için v!küfür-engelle aç & f!küfür-engelle kapat").then(m => m.delete(10000));
  ...
  console.log(`Küfür-engel komutu ${message.author.username} Tarafından kullanıldı`)
}
