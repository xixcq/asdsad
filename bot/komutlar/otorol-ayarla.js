...

{
  ...
  if (message.guild.member(message.author.id).hasPermission(0x8))

    {
      ...
      if (!mentionedRole) return message.channel.send("**Doğru Kullanım = f!otorol @<roladı> #<metinkanalı>**".then(msg => msg.delete(5000)));
...
console.log(`otorol komutu ${message.author.username} Tarafından kullanıldı`)
}
