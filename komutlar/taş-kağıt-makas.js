...
exports.run = async (client, msg, args) => {
    ...
    if (result === 'won') {
        answer = ':trophy: Başarılı, sen **Kazandın** :trophy: \nSenin Seçtiği: `' + choice + '` | Reality Cheats  \ Seçtiği: `' + botChoice + '`';
    } else if (result === 'lost') {
        answer = ':x: Bidakine **Kaybetin Dostum** :x: \nSenin Seçtiğin: `' + choice + '` | Reality Cheats  \ Seçtiği: `' + botChoice + '`';
    } else if (result === 'draw') {
        answer = ':neutral_face: It\'s a **Berabere** :neutral_face:\nSenin Seçimin: `' + choice + '` | Reality Cheats \Seçimi: `' + botChoice + '`';
    }

    msg.channel.send(answer);
  console.log(`taş kağıt makas komutu ${msg.author.username} Tarafından kullanıldı`)
};
